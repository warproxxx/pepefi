...
contract VaultManager{
    ...
    function createVault( string calldata _VAULT_NAME, uint256 _expirityDate, address[] memory _collections, uint32[] memory _ltvs, uint32[] memory _aprs, bool _external_lp_enabled, uint256 liquidityAdded) public returns (address) {
        if (liquidityAdded > 0){
        ...
        Vault vault = new Vault(_VAULT_NAME, address(this), msg.sender, _expirityDate, _collections, _ltvs, _aprs, _external_lp_enabled);
